...
- `crates/leaven-seam-service`: configured executable service implementations behind the public seam runtime. It composes `leaven-public-seam` Plan IR execution with explicitly configured local/mock/live providers, and is the home for future public-server provider wiring. It also hosts `leaven/optimize.run` by composing the `leaven-gepa` optimizer over the `leaven-run` builder (request lowering, worker-backed runner/scorer dispatch, and result projection only). It must not own stdio/HTTP framing, CLI parsing, provider protocol details, graph internals, optimizer strategy (GEPA search policy stays in `leaven-gepa`), schema validation policy, or SDK demo plans.
