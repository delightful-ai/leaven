...
- `crates/leaven-lm*`, `crates/leaven-agent*`, `crates/leaven-agentic`, `crates/leaven-agentic-*`: LLM/provider runtime surfaces, generic agentic stage adapters, and shape-specific agentic adapter helpers; these stay outside cold and engine crates. `leaven-agent-codex` is a Codex provider-family facade; `leaven-agent-codex-app-server` owns Codex app-server protocol/connectors. Do not put app-server protocol types in `leaven-agent`, `leaven-agentic`, or the facade.
