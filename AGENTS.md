...
- `crates/leaven-acp`: hot ACP stdio process/session transport owner for the locked V1 public seam. It may depend on `leaven-public-seam` for profile/method/result validation and may start external worker processes over stdin/stdout JSON-RPC. It must not own provider execution, graph mutation, MCP-over-ACP, concrete LM/agent/sandbox runtime behavior, or schema-codegen proof.
...
- `just test`: canonical full test suite; runs native Cargo workspace tests plus doctests, reports the `<30s` runtime target, and enforces the hard completion timeout configured in the root `Justfile`.
