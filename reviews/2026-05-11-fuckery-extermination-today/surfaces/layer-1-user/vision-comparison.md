...
| Work modes | Single-task, train-only, and generalization should all feel native (`docs/specs/gepa_public_private_surface.md:101-136`; `docs/specs/gepa_public_private_surface.md:722-734`). | The builder fixes non-unit case type only through `.train(Vec<C>)` (`crates/leaven-run/src/builder.rs:92-114`). The default `C = ()` path can reach `run()`, but no train/validation/test input lowers to an empty dataset/case set rather than `EvaluationSet::Unscoped` or a singleton task (`crates/leaven-run/src/builder.rs:208-222`). | No explicit single-task/no-dataset public path; empty work can masquerade as that mode. | Users fake datasets for benchmarks, environments, live evals, and single objectives, or get a run with no ordinary work attached. | Add single-task/no-dataset work input and stable cases/suites. | Scenario tests for no-dataset single-task, train-only, and generalization modes, including proof that no-train mode evaluates real work instead of an empty case set. |
...
| Cases and splits | Cases are work items with stable ids, optional targets, metadata, unique ids across default splits, and final-test-only default (`docs/specs/gepa_public_private_surface.md:773-820`). | `leaven-eval` has `Case` and duplicate-id dataset building (`crates/leaven-eval/src/dataset.rs:9-24`; `crates/leaven-eval/src/dataset.rs:95-100`), but Layer 1 does not re-export or lower through it. `run()` concatenates vectors, builds `Dataset::from_ordered`, and generates dense positional case ids (`crates/leaven-run/src/builder.rs:214-222`; `crates/leaven-run/src/builder.rs:302-356`). | User identity, duplicate detection, and split evidence policy exist below the product builder but are not the ordinary path. | Reports cannot reliably identify or reproduce case-level behavior. | Primary path accepts stable `Case`/suite inputs; vector path is explicit dense-id convenience. | Dataset/split law tests plus public builder trust-policy scenario tests (`docs/specs/eval_lowering_detail.md:790-818`). |
| Runtime/cache | LM boundary is provider-neutral and response cache is separate from engine evaluation cache (`docs/specs/lm_runtime_and_response_cache.md:54-88`). Ordinary roles need policy by solver/reflector/scorer/agent. | `CachedLm` is a public wrapper type (`crates/leaven-lm-cache/src/cached.rs:6-17`) and prelude export (`crates/leaven-lm-cache/src/lib.rs:15-19`); `leaven-run` evaluator always uses `CachePolicy::Never` (`crates/leaven-run/src/evaluator.rs:61-63`); `OpenAiLm::from_env` accepts but ignores a `default_model` argument (`crates/leaven-lm-openai/src/client.rs:27-37`). | There are pieces, not an ordinary runtime role story or role identity contract. | Users manage caching outside the run, learn wrappers too early, or misread provider/model identity. | Add role-based runtime/cache config; keep wrappers advanced; make provider/model fingerprint identity explicit. | Mock LM cache scenario by role, provider identity tests, and cache/cost summary in result. |
...
| Result/report | Result exposes optional best, stop reason, budget, graph-backed report, public events, GEPA summary, final-test semantics, durable resume status, and no ordinary `RunGraph` requirement (`docs/specs/gepa_public_private_surface.md:1184-1228`). | Engine truth has optional best and stop reasons (`crates/leaven-engine/src/engine.rs:117-184`; `crates/leaven-engine/src/engine.rs:306-309`; `crates/leaven-engine/src/events.rs:16-23`), and `OptimizeStore` can carry checkpoint persistence (`crates/leaven-run/src/store.rs:10-17`; `crates/leaven-run/src/store.rs:47-75`). Layer 1 drops that: `OptimizeResult` requires `best`, clones artifacts, has no stop/resume surface, and `OptimizationReport` holds aggregate floats and event strings (`crates/leaven-run/src/result.rs:6-61`). Missing train evidence becomes `0.0` (`crates/leaven-run/src/builder.rs:452-457`; `crates/leaven-run/src/result.rs:64-71`). | The report is a thin snapshot and collapses absence/failure into numeric values while discarding stop/resume truth. | Users cannot tell what won, why, what stopped, what evidence exists, whether a run can resume, or whether test influenced selection. | Make result/report graph-backed and truth-preserving. | Optional best, stop reason, absent/failed score, case delta, final-test-only, cache/cost, resume fingerprint, and public event tests. |
...
- The work input contract does not support ordinary single-task/no-dataset mode
  or stable case ids by default. The unit-default no-train path is an empty
  case-set path, not a real single-task contract.
...
- Provider/model identity is not stable enough for ordinary runtime roles while
  `OpenAiLm::from_env` ignores its model argument.
- The canonical AIME example proves a fixed edit and provider bypass.
- The result facade is not the ordinary run-truth surface and does not expose
  stop/resume truth already present lower down.
...
empty unit case set     != single-task work
...
ignored default model   != provider/runtime identity
