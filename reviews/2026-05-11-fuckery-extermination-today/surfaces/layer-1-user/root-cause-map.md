...
- current implementation: `optimize(seed)` initializes empty train/validation/test
  ...
  after `optimize(seed)` is `.train(Vec<C>)` (`crates/leaven-run/src/builder.rs:92-114`).
  The implemented public methods are `.validation`, `.test`, `.runner`, `.score`,
  `.using`, `.budget`, `.on_event`, `.store`, and `.run`
...
- required proof/tests: Layer 1 scenario tests for single-task optimization,
  ...
  and resume fingerprint mismatch.
...
- current implementation: `run()` concatenates split vectors, builds
...
- required proof/tests: cache-policy contract tests remain in `leaven-lm-cache`
  ...
  construction rather than example architecture changes.
...
- current implementation: `OptimizeResult` requires a `CandidateId` best, cloned
...
- blocker/gap: the original engine/report principle is that reports point at the
  ...
  semantics (`docs/specs/eval_lowering_detail.md:780-789`).
