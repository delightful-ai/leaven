...
- `import_trace2skill_eval_results.py`: no-spend importer that converts official SpreadsheetBench evaluator JSON into Leaven result JSONL only after pre-write result-intake validation against the generated runbook and per-case evaluator ids match the declared upstream dataset range.
...
- `check_trace2skill_importer_fixture.py`: no-spend ARA checker for the official-eval importer fixture; verifies positive fixture import rows carry runbook/approval/prompt-artifact provenance and refusal paths reject missing prompt artifacts, missing stage ids, wrong stage labels, missing approval evidence, and accidental full-paper reproduction labels before output rows can land.
