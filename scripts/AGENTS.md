...
- `test-suite-sla.py`: builds and discovers default workspace libtest binaries with Cargo, runs those binaries plus doctests under the suite deadline, reports the `<30s` suite target, and enforces a hard completion timeout. It prewarms doctests before starting the runtime timer and excludes milestone examples from the default lane.
