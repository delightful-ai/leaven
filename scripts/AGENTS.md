...
- `check_trace2skill_dataset_manifest_freshness.py`: no-spend ARA checker that regenerates the 400-row SpreadsheetBench dataset manifest in a temp directory and verifies the committed manifest is not stale.
...
- `check_trace2skill_one_case_result_freshness.py`: no-spend ARA checker that regenerates the deterministic one-case JSONL in a temp directory and verifies the committed result row still matches current one-case artifacts.
- `check_trace2skill_stage2_prompt_artifacts.py`: no-spend ARA checker for deterministic one-case Stage 2 rendered prompt artifacts; verifies the pending analyst prompt embeds exact upstream prompt-template files and the fanout envelope remains pending with no model response.
...
- `check_trace2skill_plot_freshness.py`: no-spend ARA checker that regenerates the Trace2Skill paper-target plot in a temp directory and verifies the committed PNG is not stale.
- `import_trace2skill_eval_results.py`: no-spend importer that converts official SpreadsheetBench evaluator JSON into Leaven result JSONL only after pre-write result-intake validation against the generated runbook, per-case evaluator ids match the declared upstream dataset range, and nested `test_cases[*].passed` derive the accepted rollup metrics (so forged 100% summaries cannot land). Refuses to overwrite an existing `--output` path.
- `check_trace2skill_result_intake.py`: no-spend ARA checker for result JSONL intake; verifies artifact paths and skill-source paths are inspectable, approval-gated rows carry runbook-required prompt artifacts and paper model/vLLM identity, command-backed paper rows invoke generated upstream command shapes with matching concrete `--model`, `--seeds`, `--workers`, `--max_turns`, and skill-evolution `run_error_analysis.py`, `run_success_analysis_llm.py`, and `skill_evolver.run_parallel_skill_evolution` segment-specific model/worker/seed/merge flags in `source_command`, range-bearing rows carry matching source-command start/end fragments, official evaluator source metrics match row/overlay identity, mechanics/one-case rows cannot overlay, and overlay rows use denominator-appropriate units/classifications plus target-provenance labels, model-family labels, and parallel-panel series-family labels.
- `check_trace2skill_importer_fixture.py`: no-spend ARA checker for the official-eval importer fixture; verifies positive fixture import rows carry runbook/approval/prompt-artifact provenance and refusal paths reject forged rollups that contradict nested test cases, existing output overwrite, missing prompt artifacts, missing stage ids, wrong stage labels, missing approval evidence, and accidental full-paper reproduction labels before output rows can land.
...
- `check_trace2skill_approval_state.py`: no-spend ARA checker that verifies the full-run approval packet is either runnable or intentionally blocked with matching closeout/status docs.
...
- `check_trace2skill_closeout_freshness.py`: no-spend ARA checker that regenerates the closeout audit in a temp directory and verifies committed JSON/Markdown outputs still match current approval, result-intake-valid result rows, dataset, and one-case state.
