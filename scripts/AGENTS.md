...
- `test-suite-sla.py`: runs workspace libtest binaries plus doctests and enforces the `<30s` suite SLA. It skips Rust targets whose source has no test markers, builds/discovers binaries before starting the runtime timer, starts known slow libtest binaries first, runs multiple binaries concurrently with a default outer fanout of 6, and preserves each binary's normal libtest thread pool.
