...
- `check_trace2skill_result_intake.py`: no-spend ARA checker for result JSONL intake; verifies artifact paths and skill-source paths are inspectable, approval-gated rows carry runbook-required prompt artifacts, mechanics/one-case rows cannot overlay, and overlay rows use denominator-appropriate units/classifications.
