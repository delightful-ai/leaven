...
P8 owns the example artifact, edit surface, deterministic AIME-shaped cases, cache loader, native OpenAI solver wiring through `leaven-lm-openai`, and public `leaven::optimize(...).train(...).validation(...).test(...).runner(...).score(...).using(...).run()` demonstration. Reusable GEPA, surface, runner, scoring, or provider abstractions belong in the owning crates.
...
- Keep the OpenAI solver integration on the same async `.runner(...)` surface
  and concrete `leaven-lm-openai` provider as reflection. Do not reintroduce a
  Python process boundary or move OpenAI Responses payload details into the
  public GEPA example surface or provider-neutral crates.
...
- If you change the live solver path, preserve native Leaven LM/runtime role
  construction and metered `RunOutput` cost. Do not add provider-specific
  behavior beyond the local example adapter.
