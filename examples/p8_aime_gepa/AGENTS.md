...
- `AimePromptSurface` is the local artifact/surface handhold: one `"system"`
  part, one `ReplaceSystem` edit, and artifact-native `AimePromptChange`.
...
## Proof Paths
- Clean deterministic proof: `just milestone-p8` with `LEAVEN_AIME_CACHE` and `LEAVEN_AIME_LIVE_OPENAI` unset proves public builder mechanics, split reporting, and scripted score movement around the current fixed-edit reflector. It does not prove real GEPA reflection.
...
- Unit tests in `src/main.rs` prove deterministic improvement, train-only
  ...
  `deterministic_aime_acceptance_shows_public_gepa_improvement` is name-bait
  after the audit: it proves fixed-edit mechanics until the reflector is real.
...
- Real product proof for this example requires an evidence-aware reflector/proposer that consumes selected feedback/trace context through Leaven-owned LM or agent runtime roles. Until then, keep this classified as mechanics-smoke/proxy-demo.
