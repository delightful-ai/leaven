...
- `aime_stage_reflector` is the deterministic agent-backed reflector. It uses
  `FakeAgentRuntime` over the production `GepaStageProposer` / `ReflectRequest`
  / `StageOutputParser` path, reads `focus/request.json`, and writes
...
- `run_openai_solver` is an external process runner around
  `scripts/openai_solver.py`. It is intentionally a live-provider smoke path
  outside Leaven LM/cache/runtime roles.

## Proof Paths
- Clean deterministic proof: `just milestone-p8` with `LEAVEN_AIME_CACHE` and `LEAVEN_AIME_LIVE_OPENAI` unset proves public builder mechanics, split reporting, and the production agent-backed GEPA reflection route with a fake runtime. It does not prove live provider quality.
...
- Live provider proof: `OPENAI_API_KEY=... LEAVEN_AIME_LIVE_OPENAI=1 LEAVEN_AIME_CACHE=target/leaven-aime-cache/aime.json cargo run -p p8_aime_gepa` swaps only the runner to the OpenAI Responses script. It spends provider resources, bypasses Leaven's provider-neutral LM/cache roles, and is not part of the cheap milestone lane.
- Unit tests in `src/main.rs` prove deterministic improvement, train-only
  ...
  public builder path now uses the agent-backed reflection route.
...
- Live-provider product proof still requires a non-fake agent runtime. The
  deterministic path is production Leaven plumbing with fake execution, not
...
- If you change the deterministic cases, preserve why baseline fails and the
  fixed edit succeeds. Otherwise the example stops proving builder/report
...
- A passing deterministic p8 run proves public API mechanics, invariant
  reporting, and the agent-backed GEPA reflection route; it is not evidence of
...
- `FakeAgentRuntime` is still a deterministic runtime fixture. Do not cite P8
  as proof of provider-native tool policy, approval handling, or live model
  reflection.
