...
- `p5_skill_paper_reproductions`: local `AGENTS.md`; no-spend mechanics surface for paper-close replication denominators. `cargo test -p p5_skill_paper_reproductions --test evoskill_manifest` proves the EvoSkill replica manifest schema records source artifacts, split blockers, scorer/frontier configuration, and proxy rejections without claiming paper-close.
