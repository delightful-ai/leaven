...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    block_on(async {
        ...
        let mut optimizer = GepaParityOptimizer {
            gepa: Gepa::new(PartMapSurface, ParetoFrontier::by_case().build()),
            proposer: ReflectiveMutation::new("improved answer".to_owned()),
            seed,
            best: None,
            done: false,
        };
...
struct GepaParityOptimizer {
    ...
    proposer: ReflectiveMutation<String>,
    seed: CandidateId,
...
impl Optimizer<PartMapProblem> for GepaParityOptimizer {
    async fn step(
        ...
        let baseline_events = self.gepa.population_mut().observe_casewise_scalar(
            self.seed,
            baseline.assessment,
            &baseline_evidence,
        );
        ...
        let events = self.gepa.population_mut().observe_casewise_scalar(
            candidate,
            screened.assessment,
            &candidate_evidence,
        );
