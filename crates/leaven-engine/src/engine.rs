...
use crate::{
    BudgetLedger, Callback, CaseSet, DynCallback, DynEvaluator, DynStopper, ErrorPolicy,
    EvaluationCache, Evaluator, Optimizer, OptimizerError, ReadScope, RunCheckpointRequest,
    RunContext, RunContextError, RunEvent, RunGraph, RunGraphView, RunPersistence, StepStatus,
    StopReason, Stopper, TrustPolicy,
};
...
impl<P: OptimizationProblem> Engine<P> {
        ...
        O: Optimizer<P>,
    {
        ...
        self.finish(optimizer.best_candidate(self.view()))
            .map_err(|error| {
                let error =
    ...
    fn stop_reason(&self) -> Option<StopReason> {
        ...
        self.stoppers.iter().find_map(|stopper| match stopper {
            ...
            EngineStopper::MetricCalls(limit) => {
                (budget.spent.metric_calls >= *limit).then_some(StopReason::BudgetExceeded)
            }
        })
    }
