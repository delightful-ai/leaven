    ...
    C: Clone + Send + Sync + 'static,
{
    ...
    let baseline_validation = if has_validation {
        let (summary, evaluation_cost) = final_eval(
            engine,
            case_set,
            store,
            seed,
        ...
        .await?;
        cost = cost.combine(&evaluation_cost);
        Some(summary)
    } else {
        None
    };
    ...
    C: Clone + Send + Sync + 'static,
{
    ...
    let baseline_train = latest_average_for_partition(&view, store, inputs.seed, &train_partition)?;
    let optimized_train =
        latest_average_for_partition(&view, store, inputs.best, &train_partition)?;
    ...
    C: Clone + Send + Sync + 'static,
{
    ...
    for assessment in view.assessments(candidate).iter() {
        ...
        latest = assessment_summary(view, store, assessment.id())?.average_score;
    }
    Ok(latest)
}
