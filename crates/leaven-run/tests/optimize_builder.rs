...
#[test]
fn run_builder_sqlite_cache_keeps_only_search_rows_at_search_checkpoint() {
    ...
    assert_eq!(
        cached_entries, 1,
        "final-report-only rows are not resume-authoritative unless their assessment ids are present in the latest graph checkpoint",
    );
    cleanup_path(&run_dir);
}
