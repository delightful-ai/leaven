## Boundary
This crate owns the Agent Skills artifact family: `SkillBank`, `SkillFolder`, `SkillFile`, validated skill names/paths, `SKILL.md` parsing, metadata, change records, and explicit folder/manifest/file surfaces.
...
## Proof Anchors
- `cargo nextest run -p leaven-artifact-skill --test skill_artifact` proves skill name/path validation, `SKILL.md` parsing, folder/bank invariants, rollback, rename semantics, content identity including permissions, and all three skill surfaces.
