...
use leaven_gepa::{
    ...
    SelectBestCandidate, StrictImprovement, SurfaceProposer, fixtures::FixedEditProposer,
...
fn gepa_owns_surface_and_lowers_selected_part_edits() {
    ...
    let mut gepa = Gepa::new(
        ...
        FixedEditProposer::new("unused".to_owned()),
    );
    let mut proposer = FixedEditProposer::new("improved".to_owned());
...
fn gepa_checkpoint_state_restores_loop_and_selector_cursor() {
    ...
    let state = gepa
        ...
        .unwrap();
    let policy = <Gepa<PartMapSurface, ParetoFrontier, FixedEditProposer<String>> as CheckpointableOptimizer<
        SmokeProblem,
...
fn gepa_builder_default_reflector_path_uses_pareto_frontier_defaults() {
    let gepa = Gepa::builder()
        ...
        .reflector(FixedEditProposer::new("improved".to_owned()))
        .max_iterations(2);
