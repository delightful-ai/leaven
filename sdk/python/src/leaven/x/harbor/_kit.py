...
def materialize_agent_kit(kit: object, target_dir: Path) -> Path:
    ...
    for skill in kit.skills:
        checked = AgentKitSkill(path=skill.path, content=skill.content)
        skill_path = target_dir / KIT_SKILLS_DIR / checked.path
        skill_path.parent.mkdir(parents=True, exist_ok=True)
        skill_path.write_text(checked.content, encoding="utf-8")
    return target_dir
