...
```bash
uv sync                           # one-time
uv run python examples/01_environment.py
...
| # | File | Shows |
|---|------|-------|
| 01 | `01_environment.py` | Every slot `lv.environment(...)` accepts: workspace, multi-role LMs, multi-role agents, sandbox, trust profile, budget, cache. |
