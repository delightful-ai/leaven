...
| Parallel proposals | Optimize-anything supports parallel proposals when `num_parallel_proposals` is configured; the current AIME source does not configure it, so source parity is serial proposals with parallel case evaluation. The local CAIS AIME checkpoint still has 10 candidates and 621 metric calls despite the documented 500-call cap | Leaven evaluates cases in parallel, but proposals are serial | Core GEPA and current-source AIME parity can remain serial. Keep the checkpoint as an artifact/source conflict until `run.log` is recovered or a future profile explicitly targets `num_parallel_proposals` fanout |
...
test_plain_text_parser_matches_upstream_output_extractor_cases
