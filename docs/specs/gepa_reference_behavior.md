...
| Parallel proposals | Optimize-anything can run parallel proposals; the local CAIS AIME checkpoint has 10 candidates and 621 metric calls despite the documented 500-call cap, which is consistent with scheduled proposal/evaluation work overshooting the nominal cap | Leaven evaluates cases in parallel, but proposals are serial | Core GEPA parity can remain serial, but optimize-anything AIME parity must label this as a proposal-fanout non-parity gap until implemented or disproven by recovered `run.log` |
