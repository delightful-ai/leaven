...
- Important scope limit: the workbook side effect is outside the ACP result contract. The worker receives `LEAVEN_TRACE2SKILL_RUN_DIR`, writes `13-1_output.xlsx` by filesystem path, and the host scorer reads that run artifact independently. This proves the ACP-launched worker produced a benchmark-accepted workbook; it does not prove ACP receipts or artifact capture bind the output workbook.
