...
- The ACP request now carries execute-mode Plan IR shaped as a real `agent_run` task: materialize a candidate workspace, run the external worker against that workspace handle, enforce a no-shell tool policy, require a file output contract for `13-1_output.xlsx`, declare `workspace.file` inputs, and use `graph_writes_atomic` commit semantics.
...
- Follow-up resolution after review: the old dry-run literal request was replaced by execute-mode Plan IR with `workspace_materialize` plus `agent_run`; the Python worker asserts that shape before solving. This proves the external worker boundary can receive and act on the real task envelope instead of a decorative literal.
- Remaining scope limit: this benchmark still does not execute the Plan IR through the public-seam host harness or engine `RunContext`; the ACP request is schema/semantics-validated and delivered over stdio, and the worker enforces the task fields it needs. Full public-seam Plan IR lowering/execution with workspace, tool-policy, output-contract receipts, and graph mutation remains neighboring evidence, not proven by this benchmark.
