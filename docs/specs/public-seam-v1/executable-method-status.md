...
| `leaven/evaluation.request` | Configured local write receipt plus RunContext callback proof | `leaven-seam-service::service`; `leaven-acp-stage-bridge::graph_host` | `leaven seam serve --stdio` executes Plan IR `request_evaluation` through the configured service, enforces `evaluation.request` capability scope, constructs and validates an `EvaluationJobDocument`, validates the context-bound request-evaluation receipt, and returns an ACP-facing `evaluation_request_receipt`. The bidirectional stage bridge additionally proves a worker-initiated `leaven/evaluation.request` callback is lowered by a host-owned typed request parser into `RunContext::request_evaluation`, returns a receipt-bound extension result, and persists the evaluation request in `RunGraph`. |
