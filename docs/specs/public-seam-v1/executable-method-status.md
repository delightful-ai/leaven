...
| `leaven/optimize.run` | Mock-configured and configured local | `leaven-seam-service::optimize_run_service` plus `leaven-gepa` | Client->host optimization dispatch executing the real `leaven-gepa` loop. `ConfiguredSeamService` lowers the locked `leaven.optimize_run.v1` request into the `leaven-run` builder, runs the GEPA frontier/reflection/admission loop, dispatches per-case runner and scorer stages to the configured `SeamStageConfig::CommandRunner` worker over `leaven/stage.run`, services nested `leaven/lm.complete` callbacks and capability-gated case reads, and projects the durable `Optimized` result plus GEPA frontier into a `leaven.optimize_run.v1` result document that revalidates before return. Deterministic tests prove a changed child is applied and re-evaluated onto the frontier with the exact reference metric-call count (1 seed + 3 parent + 3 child + 1 validation = 8), target isolation (runner refuses `case.target`, scorer serves it with a receipt), worker effect-cost aggregation, and budget-truthful stopping. The Git-backed `agent_kit` loop has the same deterministic law over a `GitProgramArtifact` with agentic reflection (`optimize_run_drives_the_real_git_backed_agent_kit_loop_with_agentic_reflection`). The seed and frontier artifact records parse into a typed payload discriminated by `artifact_type`: a `prompt` template, or an `agent_kit` projection of a Git-backed AgentKit revision (system_prompt slot content plus path-validated skill files mounted under the Codex `.agents/skills` mount); the wire layer rejects an agent_kit projection missing `system_prompt` or carrying a skill path that is absolute or contains parent traversal, and the host owns repository construction and readback so the wire record is a projection, not the artifact. V1 executes two artifact types over the `instance` objective: `prompt` runs the LM-reflection loop, and `agent_kit` runs the Git-backed agentic-reflection loop. The artifact type fixes the reflection kind: `prompt` requires `lm` (and refuses `agentic` naming `lm`); `agent_kit` requires `agentic` (and refuses `lm` naming `agentic`). For `agent_kit`, the host constructs a run-scoped bare Git repo with a deterministic seed commit from the wire projection (`leaven-agentic-git::build_program_seed`), runs the loop over a `GitProgramArtifact` with `GepaGitProgramAgenticReflector` over the real `GitProgramPathSurface`, projects each candidate revision into the runner payload under the `candidate_agent_kit` key (sibling of the prompt path's `candidate_template`) by reading the revision back to flat parts, and reads evolved child revisions back into the wire `agent_kit` shape for the result. The agentic reflector materializes the parent kit into a disposable agent workspace, runs the configured agent runtime, and reads a typed `GitProgramChange` back; the live path uses `SeamAgentConfig::CodexCli` (`gpt-5.4-mini`), and an agentic `agent_kit` run without a configured agent runtime returns the method-unavailable style. The `agent_kit` path's deterministic mechanics are proven (a scripted `FakeAgentRuntime` authors a changed `system_prompt.md` that is applied through `RunContext` and re-evaluated onto the frontier, the scorer per-case feedback reaches the rendered reflection instructions, and the evolved kit is visible in the projected `agent_kit` result). The Python SDK drives this `agent_kit` path through `leaven/optimize.run` (`sdk/python/examples/codex_terminal_bench`, 2026-06-11): each rollout runs one live Harbor `Trial` with `@openai/codex` (`gpt-5.4-mini`) installed in-container on a git-pinned Terminal-Bench-2 task, the kit's `system_prompt` is materialized as the in-container `AGENTS.md`, and the host's agentic reflector runs live Codex (`SeamAgentConfig::CodexCli`, `gpt-5.4-mini`) to author each child kit. The served path is verified functional end to end live (kit upload to `/app/AGENTS.md`, in-container codex solve, verifier reward/CTRF, Git-backed kit loop, durable run), and its deterministic mechanics are proven no-spend through that same served path with a scripted fake-codex reflection binary plus an explicit fake-trial seam (`examples/codex_terminal_bench/tests/test_kit_optimization_mechanics.py`, since the Rust `FakeAgentRuntime` is `#[cfg(test)]`-only and unreachable through the served `SeamServiceConfig`). A live kit child that strictly beats the seed was NOT yet demonstrated: a strong in-container model solves the chosen self-contained TB2 tasks regardless of the `AGENTS.md` kit (the kit is redundant with the self-contained task instruction), so a weak-but-honest seed kit leaves no headroom; a live kit-gated 0->1 cutoff needs a task whose required method is not in the instruction, or a weaker in-container model (recorded as a truthful blocker in `docs/working-memory/gepa-over-seam-continuation.md`). Other artifact types and objectives are refused with a message naming what is supported. `optimizer.population_size` is honored as the candidate-pool cap (a stop condition over the seed plus loop-authored children, stopping the loop like budget exhaustion) and `optimizer.minibatch_size` is honored as the train screening minibatch override; a `population_size` of 1 is refused naming the `>= 2` service bound because a cap of 1 admits only the seed and can never author a child. Budget facts: `optimizer.max_metric_calls` always bounds the loop, and the optional `optimizer.max_cost_usd_micro` adds a `usd_micro` cost-axis ceiling enforced by the engine budget ledger; a deterministic test (2026-06-10) proves a usd ceiling below one child's screening spend stops the loop with the seed as best. A missing stage-worker argv returns the method-unavailable style. OpenAI-backed reflection through `SeamLmConfig::OpenAi` executes under the host's tokio loop runtime; a deterministic no-network test drives the full loop with the `OpenAi` reflector against a loopback fake provider. A credentialed live run through the Python SDK (`sdk/python/examples/14_live_optimize_aime.py`, 2026-06-11) drove the full loop on real AIME data with a `gpt-4.1-mini` solver and `gpt-5.4-mini` reflection served by one `SeamLmConfig::OpenAi` provider (the provider honors the per-call model the solver `lm.complete` and the reflection request each carry, since `SeamLmConfig::OpenAi` has no pinned model): the seed scored 0.0, GEPA authored a changed reasoning instruction, screened it past the parent on the train minibatch, admitted it, and re-validated it to 1.0 onto the frontier in 12 metric calls. USD cost reports 0.0 because Leaven meters token usage, not per-model dollar pricing, so the run is bounded by `metric_calls`, not the optional usd ceiling. Unlike worker-profile methods, this is a client->host dispatch, not a worker callback or stage dispatch, so the worker profile does not advertise it. |
