...
- The checker also creates otherwise shape-valid approval-gated overlay rows
  under both `target/` and top-level ARA `results/` and expects the plotter to
  fail while `full_run_plan.md` remains blocked, then removes the temporary ARA
  row.
- The documented paper-target command remains dependency-minimal:
  ...
  for approval-gated overlay rows.
...
- Validates aggregate provenance against the named runbook stage's generated
  ...
  `serving_backend`, `metric_name`, and `metric_unit`.
...
- The checker exercises those refusal paths directly: missing runbook stage id,
  ...
  full-paper case-count drift, full-paper metric-value drift, full-paper missing
  required split-range source coverage, full-paper source model drift, subset
  seed drift, subset worker drift, full-paper serving/model drift with null plot
  binding, blocked
