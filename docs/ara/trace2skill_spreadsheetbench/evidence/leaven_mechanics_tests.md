...
| `cargo test -p trace2skill_spreadsheetbench --test one_case_run` | Scenario | one-case staged-run mechanics | Exact case `13-1` can be staged into a durable run directory, scored after an output workbook and transcript exist, and converted into a pending Stage 2 analyst fan-out with upstream prompt template material. | Does not execute the live spreadsheet agent, analyst model call, patch parser, hierarchical merge, or paper metric evaluation. | C06, C07 |
